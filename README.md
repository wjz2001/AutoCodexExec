...
### 权限不足与批准的关系
  ...
  2. 某项操作无法在沙箱内完成时，Codex 只负责写入 PERMISSION_REQUEST.json，然后退出。请求必须写明权限来源、实际拒绝原因，以及具体文件、目录、服务、系统资源；
  3. 监督脚本停止启动新任务，在当前终端显示权限来源、原因、涉及位置、执行目录、风险，并等待你的决定，不会自动批准。如果已在脚本顶部填写 WAV Base64，此时会播放声音，并每 5 分钟重复提醒一次。Base64 应放在引号内；
...
## 控制台输出
  ...
  控制台只显示经过整理的彩色任务摘要、思考、命令结果、权限详情和错误末尾，不直接输出 Codex JSON
